...
import GoogleLoginButton, { useAuth } from "~/auth/google_login";
    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_DOMAIN}/booking/list?search=${search}`,
        );
        ...
        const formattedEvents = data.map(
          (item: { desc: any; start_at: any; end_at: any }) => ({
            title: item.desc,
            start: item.start_at,
            end: item.end_at,
    ...
    try {
      ...
      const newEvent = {
        title: selectedServices
          .map((id) => tags.find((tag) => tag.value === id)?.label)
          .join(", "),
        start,
        end,
      ...
      {showModal && selectedTime && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded shadow-md w-80">
            ...
            <p>
              時間：{formatTime(selectedTime.start)} ~ {formatTime(selectedTime.end)}
            </p>
